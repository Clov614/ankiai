...
- 🎴 **会话制卡**：解释面板点「🎴 生成卡片」，AI 自动把当前对话提炼成 **EnWords 单词卡**（单词/音标/词性/释义/CEFR/例句/AI解析/记忆钩子/单词发音），复选后写入**任意牌组**，重复词条自动跳过；首次使用会自动创建 EnWords 笔记类型（与 [LanguagePreviewAgentFlow](https://github.com/Clov614) 的最新模板一致）
...
划选卡片文字 → 右键 AI 解释，面板即出解析；划选面板内容可继续朗读 / 追问：

![AnkAI 主界面：复习卡片 + AI 解释面板 + 右键菜单](docs/images/hero-main.jpg)

多轮追问：把面板里划选的内容引用到输入框，继续追问：

![多轮追问：引用划选 → 发送](docs/images/followup.jpg)

会话制卡：从解释会话里提炼单词卡，勾选后一键加入牌组：

![会话制卡](docs/images/card-generation.jpg)
...
4. **制卡**：解释面板有对话内容后点「🎴 生成卡片」→ AI 提炼候选 → 勾选（右侧可改释义/例句/解析）→ 选牌组（可输新名创建）→ 添加；会话里讨论过的词条、短语、句型都能成卡
...
└── ankiai_lib/
    ...
    ├── cardgen.py       # 会话 → 卡片候选（LLM 抽取 + JSON 健壮解析）
    ├── cards_dialog.py  # 制卡对话框（候选复选 + 详情编辑 + 牌组选择）
    ├── notes.py         # 集合写入：EnWords 模板引导、查重、批量加卡
    ├── entemplate.py    # EnWords 笔记类型定义（字段/正反面/样式/自动播放）
    ├── settings.py      # 设置对话框
    ├── history.py       # 解释历史存取
